...
import { DEFAULT_NOTIFICATION_TEMPLATES } from "../src/data/sample-messages";
...
async function main() {
  ...
  for (let i = 1; i <= 18; i += 1) {
    ...
    if (["hr_pass", "hr_failed"].includes(status)) {
      await Assessment.create({
        ...
        comment: status === "hr_failed" ? "Needs stronger communication clarity." : "Strong culture fit.",
  ...
  // Ensure candidate1 login account is convenient
  ...
  console.log({
    ...
    password: "Password123! (superadmin uses SEED_SUPERADMIN_PASSWORD or SuperAdmin123!)",
  });
}
